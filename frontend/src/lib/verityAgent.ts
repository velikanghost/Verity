...
export function reviewPredictionPost(input: MarketInput): VerityAgentReview {
  ...
  if (!question) {
    findings.push({ severity: "blocker", message: "Add a clear YES/NO market question." });
  }

  if (question && !question.endsWith("?")) {
    findings.push({ severity: "warning", message: "Phrase the prediction as a question so voters scan it quickly." });
  }

  if (question && !hasNumber(`${question} ${yesCondition} ${noCondition}`)) {
    findings.push({ severity: "blocker", message: "Add a measurable number, percentage, date, price, or count." });
  }
  ...
  if (!input.deadline || deadlineDays === null) {
    ...
    findings.push({ severity: "warning", message: "Very short deadlines may not gather enough social signal." });
  }

  if (!resolutionSource) {
    findings.push({ severity: "blocker", message: "Name the source that will resolve the market." });
  }
