...
import { PvpTicket, PvpTicketDocument, PvpMatch, PvpMatchDocument } from "./pvp.model"
...
export class PvpService {
  ...
  constructor(
    @InjectModel(PvpTicket.name) private pvpTicketModel: Model<PvpTicketDocument>,
  ...
  async createPvpEvent(adminId: string, dto: CreatePvpEventDto) {
    ...
    this.logger.log(`Admin ${adminId} successfully deployed PvP Event: ${parentMarket._id} with 7 child options.`)
  ...
  async submitTicket(userId: string, dto: SubmitTicketDto) {
    ...
    if (new Date() >= parent.deadline) {
      throw new BadRequestException("Event deadline has passed. Predictions are locked.")
    }
    ...
    if (existing) {
      if (existing.status === "matched") {
        throw new BadRequestException("You have already been matched with an opponent for this match. Selections are locked.")
      }
    ...
    if (user.doubleBoostRemaining > 0) {
      ...
      this.logger.log(`User ${userId} consumed a double boost. remaining: ${user.doubleBoostRemaining}`)
    }
  ...
  async matchmake(ticket: PvpTicketDocument): Promise<PvpMatchDocument | null> {
    ...
    for (const candidate of candidates) {
      ...
      for (const pick of ticket.picks) {
        const candidatePick = candidate.picks.find(p => p.marketId.toString() === pick.marketId.toString())
    ...
    // Emit Socket events
    this.socketGateway.broadcastToRoom(`user:${ticket.userId.toString()}`, "pvp-matched", { matchId: match._id.toString() })
    this.socketGateway.broadcastToRoom(`user:${bestOpponent.userId.toString()}`, "pvp-matched", { matchId: match._id.toString() })
    ...
    this.logger.log(`Matched tickets: ${ticket._id} and ${bestOpponent._id} inside match: ${match._id}`)
    return match
  }

  async resolvePvpMatchesForMarket(marketId: string, winningOutcome: "YES" | "NO") {
    ...
    this.logger.log(`Resolving child market ${marketId} outcome: ${winningOutcome} on ${tickets.length} PvP tickets.`)
  ...
  private async resolveMatch(match: PvpMatchDocument, ticket1: PvpTicketDocument, ticket2: PvpTicketDocument) {
    this.logger.log(`Resolving PvP match ${match._id} for users ${match.user1Id} and ${match.user2Id}`)
    ...
    if (score1 > score2) {
      ...
      const time1 = ticket1.createdAt ? new Date(ticket1.createdAt).getTime() : 0
      const time2 = ticket2.createdAt ? new Date(ticket2.createdAt).getTime() : 0
      if (time1 < time2) {
        winnerId = match.user1Id
      } else if (time2 < time1) {
        winnerId = match.user2Id
    ...
    if (user1.referredById) {
      ...
      this.logger.log(`Referral Kickback of ${kickback1} XP awarded to referrer of user ${user1._id}`)
    }
    if (user2.referredById) {
      ...
      this.logger.log(`Referral Kickback of ${kickback2} XP awarded to referrer of user ${user2._id}`)
    }
    ...
    this.socketGateway.broadcastToRoom(`user:${user1._id.toString()}`, "pvp-resolved", { matchId: match._id.toString() })
    this.socketGateway.broadcastToRoom(`user:${user2._id.toString()}`, "pvp-resolved", { matchId: match._id.toString() })

    // In-app Notifications
    const u1 = user1.username
    const u2 = user2.username
    const res1 = winnerId ? (winnerId.toString() === user1._id.toString() ? "WON 🏆" : "LOST ❌") : "TIED 🤝"
    const res2 = winnerId ? (winnerId.toString() === user2._id.toString() ? "WON 🏆" : "LOST ❌") : "TIED 🤝"
  ...
  private async awardFirstWinBoosts(referee: UserDocument) {
    ...
    this.logger.log(`Co-Op double boosts (+2) awarded to referee ${referee._id} and referrer ${referrer._id}`)
  }

  async getPvpStatus(userId: string) {
    // Find the latest active ticket (either queued or matched)
    const ticket = await this.pvpTicketModel.findOne({
      userId: new Types.ObjectId(userId),
      status: { $in: ["queued", "matched"] },
    }).sort({ createdAt: -1 })
    ...
    if (ticket.matchId) {
      ...
      if (match) {
        opponentTicket = await this.pvpTicketModel.findById(ticket.opponentTicketId)
    ...
    const children = await this.marketModel.find({ parentMarketId: ticket.parentMarketId })
    ...
    return {
      ticket: {
        ...
        picks: ticket.picks.map(p => {
          const matchChild = children.find(c => c._id.toString() === p.marketId.toString())
          return {
      ...
      opponent: opponent ? {
        ...
        picks: opponentTicket ? opponentTicket.picks.map(p => {
          const matchChild = children.find(c => c._id.toString() === p.marketId.toString())
          return {
            marketId: p.marketId.toString(),
            optionName: matchChild?.optionName || "Unknown Proposition",
  ...
  async getLeaderboards() {
    // ELO (Skill)
    const eloList = await this.userModel.find({
      isOnboarded: true,
    }).sort({ eloRating: -1 }).limit(50)

    // XP (Volume)
    const xpList = await this.userModel.find({
      isOnboarded: true,
    }).sort({ arenaXp: -1 }).limit(50)
  ...
  async getMatchHistory(userId: string) {
    ...
    const matches = await this.pvpMatchModel.find({
      $or: [{ user1Id: uId }, { user2Id: uId }],
      status: "resolved",
    }).sort({ resolvedAt: -1 }).limit(30)
