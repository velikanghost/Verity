...
export class InteractionsController {
  ...
  async toggleLike(@Body() dto: ToggleInteractionDto, @Request() req: any) {
    ...
    await this.interactionsService.toggleLike(dto.postId, userId, dto.currentlyActive);
    return null;
  }
  ...
  async toggleReshare(@Body() dto: ToggleInteractionDto, @Request() req: any) {
    const userId = req.user.id;
    await this.interactionsService.toggleReshare(dto.postId, userId, dto.currentlyActive);
    return null;
  }
}
